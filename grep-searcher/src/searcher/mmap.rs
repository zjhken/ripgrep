...
impl MmapChoice {
    ...
    /// # Safety
    ///
    /// This constructor is unsafe because memory maps are an inherently unsafe
    /// construct in Rust, and there is no obvious way to encapsulate them on
    /// all platforms without simultaneously negating their benefits. As such,
    /// enabling memory maps requires the caller to write `unsafe` code.
    ///
